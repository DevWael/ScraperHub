import { CheerioCrawler, PlaywrightCrawler, PuppeteerCrawler, Dataset, Log, createCheerioRouter, createPlaywrightRouter, createPuppeteerRouter } from 'crawlee';
...
export class CrawleeCrawler {
	...
	async start(): Promise<CrawleeResult> {
		try {
	...
	private async runCheerioCrawler(): Promise<CrawleeResult> {
	...
	private async runPlaywrightCrawler(): Promise<CrawleeResult> {
	...
	private async runPuppeteerCrawler(): Promise<CrawleeResult> {
	...
	private async processPage(url: string, $: any, crawlerType: string): Promise<void> {
	...
	private async processPageWithBrowser(url: string, page: any, crawlerType: string): Promise<void> {
	...
	private async processImages($: any, baseUrl: string): Promise<any[]> {
	...
	private extractPageMetadata($: any, url: string): any {
		const metadata = {
			url: url,
			title: '',
			description: '',
	...
	private extractContentStatistics($: any, markdownContent: string): any {
		const text = $.text();
	...
	private async saveContent(url: string, metadata: any, markdownContent: string, stats: any): Promise<void> {
...
\n`;
	...
	private generateResult(): CrawleeResult {
