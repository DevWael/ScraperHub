...
async function handleRedirects(url, options = {}) {
  ...
  while (redirectCount < CONFIG.maxRedirects) {
    try {
      const response = await axios.get(currentUrl, {
        timeout: CONFIG.redirectTimeout,
        maxRedirects: 0, // Disable axios automatic redirects to handle manually
      ...
      // If we get a successful response (2xx), no more redirects
      ...
      if (response.status >= 300 && response.status < 400) {
        ...
        if (redirectHistory.some(redirect => redirect.url === redirectUrl)) {
          throw new Error(`Redirect loop detected: ${redirectUrl} already visited`);
        }
        ...
        if (CONFIG.debug) {
          console.log(`Following redirect ${redirectCount}: ${response.status} -> ${redirectUrl}`);
        }
      ...
      if (error.response && error.response.status >= 300 && error.response.status < 400) {
        // This is a redirect response, handle it
        const location = error.response.headers.location;
        if (!location) {
          throw new Error(`Redirect response (${error.response.status}) without Location header`);
        }
        ...
        if (CONFIG.debug) {
          console.log(`Following redirect ${redirectCount}: ${error.response.status} -> ${redirectUrl}`);
        }
  ...
  throw new Error(`Maximum redirects (${CONFIG.maxRedirects}) exceeded`);
}
...
async function crawl(startUrl, options = {}) {
  ...
  async function scrapePage(url, retryCount = 0) {
    ...
    try {
      ...
      let response, finalUrl, redirectHistory;
      
      if (CONFIG.followRedirects) {
        // Handle redirects manually
        const redirectResult = await handleRedirects(url, {
          userAgent: finalOptions.userAgent
        });
        response = redirectResult.response;
        finalUrl = redirectResult.finalUrl;
        redirectHistory = redirectResult.redirectHistory;
        
        // Update current URL to the final URL after redirects
        currentUrl = finalUrl;
      } else {
        // Use axios default behavior (limited redirect support)
      ...
      // Extract metadata
      const metadata = extractPageMetadata($, finalUrl);
      
      // Add redirect information to metadata if redirects occurred
      if (redirectHistory && redirectHistory.length > 0) {
        metadata.originalUrl = url;
        metadata.finalUrl = finalUrl;
        metadata.redirectHistory = redirectHistory;
        metadata.redirectCount = redirectHistory.length;
      }
...
${metadata.redirectCount ? `**Redirects:** ${metadata.redirectCount} (${metadata.originalUrl} → ${metadata.finalUrl})\n` : ''}
...
\n`;
      ...
      sitemap.push({
        ...
        statistics: stats,
        scrapedAt: new Date().toISOString(),
        redirectInfo: redirectHistory && redirectHistory.length > 0 ? {
          originalUrl: url,
          finalUrl: finalUrl,
          redirectCount: redirectHistory.length,
          redirectChain: redirectHistory.map(r => ({
            from: r.url,
            to: r.redirectTo,
            status: r.status
          }))
