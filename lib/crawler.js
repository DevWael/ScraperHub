...
async function crawl(startUrl, options = {}) {
  ...
  let outputDir = finalOptions.output || path.join('output', baseDirName);
  let dirSuffix = 0;
  while (await fs.pathExists(outputDir)) {
    dirSuffix++;
    outputDir = finalOptions.output || path.join('output', `${baseDirName}-${dirSuffix}`);
  }
  ...
  const printProgress = () => {
    ...
    // Calculate progress based on actual scraped pages vs total discovered URLs
    const totalDiscovered = visited.size + toVisit.length;
    const totalScraped = successfulPages + failedPages;
    const progress = totalDiscovered > 0 ? Math.round((totalScraped / totalDiscovered) * 100) : 0;
